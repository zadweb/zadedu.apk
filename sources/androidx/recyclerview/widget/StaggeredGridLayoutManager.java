...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            ...
            for (int i4 = 0; i4 < this.mSpanCount; i4++) {
                if (this.mLastLayoutFromEnd) {
                    i2 = this.mSpans[i4].getEndLine(RecyclerView.UNDEFINED_DURATION);
                    ...
                    i2 = this.mSpans[i4].getStartLine(RecyclerView.UNDEFINED_DURATION);
                    if (i2 != Integer.MIN_VALUE) {
                        i3 = this.mPrimaryOrientation.getStartAfterPadding();
                    } else {
                        savedState.mSpanOffsets[i4] = i2;
    ...
    public boolean areAllEndsEqual() {
        int endLine = this.mSpans[0].getEndLine(RecyclerView.UNDEFINED_DURATION);
        for (int i = 1; i < this.mSpanCount; i++) {
            if (this.mSpans[i].getEndLine(RecyclerView.UNDEFINED_DURATION) != endLine) {
                return false;
            }
        }
        return true;
    ...
    public boolean areAllStartsEqual() {
        int startLine = this.mSpans[0].getStartLine(RecyclerView.UNDEFINED_DURATION);
        for (int i = 1; i < this.mSpanCount; i++) {
            if (this.mSpans[i].getStartLine(RecyclerView.UNDEFINED_DURATION) != startLine) {
                return false;
            }
        }
        return true;
    ...
    /* access modifiers changed from: package-private */
    public class Span {
        int mCachedEnd = RecyclerView.UNDEFINED_DURATION;
        int mCachedStart = RecyclerView.UNDEFINED_DURATION;
        int mDeletedSize = 0;
        final int mIndex;
        ...
        public void cacheReferenceLineAndClear(boolean z, int i) {
            int i2;
            if (z) {
                i2 = getEndLine(RecyclerView.UNDEFINED_DURATION);
            } else {
                i2 = getStartLine(RecyclerView.UNDEFINED_DURATION);
            }
            clear();
            if (i2 != Integer.MIN_VALUE) {
    ...
    public class AnchorInfo {
        ...
        public void saveSpanReferenceLines(Span[] spanArr) {
            ...
            for (int i = 0; i < length; i++) {
                this.mSpanReferenceLines[i] = spanArr[i].getStartLine(RecyclerView.UNDEFINED_DURATION);
            }
        }
