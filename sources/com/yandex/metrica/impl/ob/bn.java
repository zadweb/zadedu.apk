...
public class bn implements Closeable {
    ...
    private long b() {
        this.b.lock();
        try {
            long a2 = a(this.d.getReadableDatabase().rawQuery("SELECT count() FROM reports", null));
            this.b.unlock();
            return a2;
    ...
    public int a(SQLiteDatabase sQLiteDatabase) {
        try {
            ...
            return sQLiteDatabase.delete("reports", String.format("%1$s NOT IN (%2$s) AND (%3$s IN (SELECT %3$s FROM %4$s ORDER BY %5$s, %6$s LIMIT (SELECT count() FROM %4$s) / %7$s ) OR %5$s < %8$s )", "type", TextUtils.join(",", numArr), "id", "reports", "session_id", "number", 10, Long.valueOf((System.currentTimeMillis() / 1000) - TimeUnit.DAYS.toSeconds(14))), null);
        } catch (Throwable th) {
    ...
    public Cursor b(long j2, bl blVar) throws SQLiteException {
        this.b.lock();
        try {
            Cursor query = this.d.getReadableDatabase().query("reports", null, "session_id = ? AND session_type = ?", new String[]{Long.toString(j2), Integer.toString(blVar.a())}, null, null, "number ASC", null);
            this.b.unlock();
            return query;
        } catch (Exception unused) {
            this.b.unlock();
    ...
    public ContentValues c(long j2, bl blVar) {
        ...
        Cursor cursor = null;
        try {
            cursor = this.d.getReadableDatabase().rawQuery(String.format(Locale.US, "SELECT report_request_parameters FROM sessions WHERE id = %s AND type = %s ORDER BY id DESC LIMIT 1", Long.valueOf(j2), Integer.valueOf(blVar.a())), null);
    ...
    static /* synthetic */ void a(bn bnVar, ContentValues contentValues) {
        if (contentValues != null) {
            bnVar.c.lock();
            try {
                bnVar.d.getWritableDatabase().insertOrThrow("sessions", null, contentValues);
            } catch (Exception unused) {
            } catch (Throwable th) {
                bnVar.c.unlock();
                throw th;
