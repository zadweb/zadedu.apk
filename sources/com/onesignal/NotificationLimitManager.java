...
class NotificationLimitManager {
    ...
    static void clearOldestOverLimitFallback(Context context, int i) {
        Cursor cursor = null;
        try {
            cursor = OneSignalDbHelper.getInstance(context).getSQLiteDatabaseWithRetries().query("notification", new String[]{"android_notification_id"}, OneSignalDbHelper.recentUninteractedWithNotificationsWhere().toString(), null, null, null, "_id", getMaxNumberOfNotificationsString() + i);
