    ...
    public final void onStop() {
        if (((Boolean) zzkb.zzik().zzd(zznk.zzavd)).booleanValue() && !this.zzdam) {
            ...
            for (zzalr zzalr : this.zzdae.zzry()) {
                String valueOf = String.valueOf(zzalr.name);
                bundle.putString(valueOf.length() != 0 ? "fps_c_".concat(valueOf) : new String("fps_c_"), Integer.toString(zzalr.count));
                String valueOf2 = String.valueOf(zzalr.name);
                bundle.putString(valueOf2.length() != 0 ? "fps_p_".concat(valueOf2) : new String("fps_p_"), Double.toString(zzalr.zzctb));
            }
            int i = 0;
            while (true) {
